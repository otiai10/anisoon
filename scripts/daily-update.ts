...
const __main__ = async () => {
    ...
    const db = { baseurl: "https://raw.githubusercontent.com/otiai10/syobocal/main/db/" };
    ...
    if (dbres.status != 200) {
        console.error(entry);
        throw new Error(`failed to fetch database: ${dbres.status} ${dbres.statusText}: ${endpoint}`)
    }
    ...
    const tracks = await Promise.all(items.map(async (item): Promise<Track> => {
        ...
        const query = new URLSearchParams({
            ...
            type: "video",
            videoDuration: "short",
            key: Deno.env.get("API_KEY") as string,
        });
        await sleep(Math.random() * 10 * 1000);
        const ytres = await fetch(YouTubeSearchAPI + "?" + query.toString());
        const response: GoogleAppsScript.YouTube.Schema.SearchListResponse = await ytres.json();
        console.log("YouTube:", ytres.status, ytres.statusText);
        if (ytres.status != 200) {
            console.error(response);
            throw new Error(`failed to fetch YouTube: ${ytres.status} ${ytres.statusText}`)
        }
        query.delete("key");
        return { ...item, youtube: {
            query: { keyword, params: query.toString(), found: response.items!.length },
            videos: (response.items || []).map(item => ({
                id: item.id!.videoId!, kind: item.id!.kind!, title: item.snippet!.title!,
                publishedAt: item.snippet!.publishedAt!, thumbnails: item.snippet!.thumbnails!,
                channel: { id: item.snippet!.channelId!, title: item.snippet!.channelTitle! },
            })),
        }}
    }));

    // Write to a file
    const dirpath = path.join("db", "tracks", ...entry.context.output.split(path.posix.sep).slice(-3,-1));
